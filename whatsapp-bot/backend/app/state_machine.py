...
from .utils import detect_lang, fmt_money
from . import booking
...
async def handle(bid, phone, text):
    text=(text or '').strip(); st=await get_state(bid,phone); lang=st.get('lang') or detect_lang(text)
    low=text.lower()
    if low in {'إلغاء','الغاء','cancel'}:
        ...
        cfg=await booking.settings_map(bid); policy=cfg.get('cancellation_policy_ar' if lang=='ar' else 'cancellation_policy_en',t(lang,'يمكنك الإلغاء قبل الموعد.','You may cancel before the appointment.'))
    ...
    if st.get('step')=='cancel_confirm':
        if low in {'نعم','yes','y'}:
            await booking.cancel(bid,st['appointment_id']); await clear(bid,phone)
            return t(lang,'تم إلغاء الحجز.','Your appointment has been cancelled.')
        await clear(bid,phone); return t(lang,'لم يتم إلغاء الحجز.','The appointment was not cancelled.')
    if st.get('step')=='reschedule_slot':
        ...
        try: ap=await booking.reschedule(bid,st['appointment_id'],slots[n-1])
        except Exception: return t(lang,'هذا الموعد لم يعد متاحاً. اختر وقتاً آخر.','That slot is no longer available. Choose another time.')
        await clear(bid,phone); d=ap['start_time'].astimezone(TZ)
    ...
    if not st:
        menu,rows=await service_menu(bid,lang); await set_state(bid,phone,{'lang':lang,'step':'service'})
        return t(lang,'أهلاً بك.\n','Welcome.\n')+menu
    if st.get('step')=='service':
        rows=await booking.services(bid); n=extract_num(text); chosen=None
        if n and 1<=n<=len(rows): chosen=rows[n-1]
        else:
            q=low
            for s in rows:
                if q and (q in s['name_ar'].lower() or q in s['name_en'].lower() or s['name_ar'].lower() in q or s['name_en'].lower() in q): chosen=s; break
        if not chosen:
            menu,_=await service_menu(bid,lang); return t(lang,'لم أفهم اختيارك.\n','I could not match that service.\n')+menu
    ...
    if st.get('step')=='slot':
        ...
        if not c or not c['name']:
            nextst['step']='name'; await set_state(bid,phone,nextst); return t(lang,'ما اسمك؟','What is your name?')
        return await finalize(bid,phone,nextst,c['name'])
    if st.get('step')=='name':
        name=text[:100]
        if len(name)<2: return t(lang,'اكتب الاسم من فضلك.','Please enter your name.')
        await booking.upsert_customer(bid,phone,name)
        return await finalize(bid,phone,st,name)
    await clear(bid,phone); menu,_=await service_menu(bid,lang); await set_state(bid,phone,{'lang':lang,'step':'service'}); return menu

async def finalize(bid,phone,st,name):
    lang=st['lang']
    try: ap=await booking.create_appointment(bid,phone,st['service_id'],st['slot'])
    except Exception:
        slots=await booking.available_slots(bid,st['service_id']); st['slots']=slots; st['step']='slot'; await set_state(bid,phone,st)
        return t(lang,'هذا الموعد تم حجزه للتو. اختر وقتاً آخر.\n','That slot was just booked. Choose another time.\n')+slots_text(slots,lang)
    await booking.upsert_customer(bid,phone,name); biz=await booking.business(bid); await clear(bid,phone); d=ap['start_time'].astimezone(TZ)
